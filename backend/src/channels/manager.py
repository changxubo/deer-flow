...
class ChannelManager:
    ...
    async def _handle_chat(self, msg: InboundMessage) -> None:
        ...
        thread_id = None
        thread_was_reused = False
        if msg.topic_id:
            thread_id = self.store.get_thread_id(msg.channel_name, msg.chat_id, topic_id=msg.topic_id)
            if thread_id:
                thread_was_reused = True
        ...
        try:
            ...
            if thread_was_reused and "404" in str(exc):
                logger.warning("Thread %s no longer exists on LangGraph Server, creating new thread", thread_id)
                thread_id = await self._create_thread(client, msg)
                assistant_id, run_config, run_context = self._resolve_run_params(msg, thread_id)
                result = await client.runs.wait(
                    thread_id,
